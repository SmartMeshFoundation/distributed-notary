...
func sign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	ScriptClass, []btcutil.Address, int, error) {
		...
		0
				IF
			OP_HASH160 {{LockSecretHash}} OP_EQUALVERIFY OP_DUP OP_HASH160 {{分布式私钥对应的PublicKeyHash}} OP_EQUALVERIFY OP_CHECKSIG
		ELSE
			{{过期的块号}} CHECKLOCKTIMEVERIFY DROP OP_DUP OP_HASH160 {{用户的PublicKeyHash}} EQUALVERIFY CHECKSIG
		ENDIF
		 */
