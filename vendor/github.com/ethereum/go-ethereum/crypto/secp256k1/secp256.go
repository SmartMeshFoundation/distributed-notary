...
var (
	...
	ErrAddPointFailed      = errors.New("add point failed")
)
...
//AddPoint 两个点相加,secp256k1_ec_pubkey_combine
func AddPoint(key1, key2 []byte) ([]byte, error) {
	var (
		pubkey     = make([]byte, 65)
		key1data   = (*C.uchar)(unsafe.Pointer(&key1[0]))
		key2data   = (*C.uchar)(unsafe.Pointer(&key2[0]))
		pubkeydata = (*C.uchar)(unsafe.Pointer(&pubkey[0]))
	)
	if C.secp256k1_ext_add_pubkey(context, pubkeydata, key1data, key2data, 65) == 0 {
		return nil, ErrAddPointFailed
	}
	return pubkey, nil
}
