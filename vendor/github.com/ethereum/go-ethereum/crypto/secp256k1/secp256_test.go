...
func TestAddPoint(t *testing.T) {
	...
	key1 := elliptic.Marshal(S256(), key.X, key.Y)
	t.Logf("key1 x=%s,y=%s", key.X.Text(16), key.Y.Text(16))
	t.Logf("keybin=%s", hex.EncodeToString(key1))
	//key1, _ := generateKeyPair()
	key2, _ := generateKeyPair()
	add, err := AddPoint(key1, key2)
	if err != nil {
		t.Error(err)
		return
	}
	...
	t.Logf("add=%s", hex.EncodeToString(add))
}
