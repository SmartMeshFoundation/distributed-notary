...
//add by bai 修正签名不符合规范问题

static int secp256k1_ext_ecdsa_signature_normalize(
	const secp256k1_context* ctx,
	unsigned char *sigoutdata,
	const unsigned char *sigindata
)  {
	secp256k1_ecdsa_signature sigin;
	secp256k1_ecdsa_signature sigout;
    //得到normalized之前的r,s
	if (!secp256k1_ecdsa_signature_parse_compact(ctx, &sigin, sigindata)) {
		return 0;
	}
	//normalize
	if (!secp256k1_ecdsa_signature_normalize(ctx,&sigout,&sigin)){
	    return 0;
	}
	//输出
	secp256k1_ecdsa_signature_serialize_compact(ctx,sigoutdata,&sigout);
	return 1;
}
