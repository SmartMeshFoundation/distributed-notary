...
/*
pubnonce 输出的pubkey
pubnonces 输入的两个pubkey
n:2
*/
static int  secp256k1_ext_add_pubkey(
    const  secp256k1_context* ctx,
    unsigned char * pubkeydataout,
    const unsigned char * pubkeydata1,
    const unsigned char * pubkeydata2,
    size_t pubkeylen
    ) {
    secp256k1_pubkey pubkey[2];
    secp256k1_pubkey pubkey_out;
    int ret=0;
    secp256k1_pubkey *tmp[2]={&pubkey[0],&pubkey[1]};
	if (!secp256k1_ec_pubkey_parse(ctx, &pubkey[0], pubkeydata1, pubkeylen)){
		return 0;
	}
	//printf("xxxxxkey1....!!! %s\n,",bin2hex(pubkey[0].data,64));
    ...
    //printf("outkey=%s\n",bin2hex(pubkeydataout,pubkeylen));
    return ret;
}
