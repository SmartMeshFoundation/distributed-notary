...
func NewJettradeTxSignDOBuyerData(req *jettradeapi.SignDOBuyerOnSpectrumRequest, nonce uint64, callerAddress common.Address, c *chainservice.SMTProxy) (data *JettradeTxSignDOBuyerData, err error) {
	data = &JettradeTxSignDOBuyerData{
		Nonce:       nonce,
	...
	// 调用合约
	_, err = c.SignDOBuyer(transactor, req.TokenId)
	if err != errShouldBe {
		// 这里不可能发生
		err = fmt.Errorf("CreateAndSignDOBuyer2 err=%w", err)
		return
	}
	return data, nil
}
...
func (d *JettradeTxSignDOBuyerData) GetSignBytes() []byte {
	return d.BytesToSign
}

// GetName : impl MessageToSign
func (d *JettradeTxSignDOBuyerData) GetName() string {
	return JettradeTxSignDOBuyerDataName
}

// GetTransportBytes : impl MessageToSign
func (d *JettradeTxSignDOBuyerData) GetTransportBytes() []byte {
...
func (d *JettradeTxSignDOBuyerData) Parse(buf []byte) error {
...
// VerifySignData :
func (d *JettradeTxSignDOBuyerData) VerifySignData(c chain.Chain, account common.Address) (err error) {
	//不做校验,跳过
	return nil
}
