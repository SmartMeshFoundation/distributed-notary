...
func NewJettradeTxIssueINVData(req *jettradeapi.IssueINVOnEthereumRequest, nonce uint64, callerAddress common.Address, c *chainservice.EthProxy) (data *JettradeTxIssueINVData, err error) {
	data = &JettradeTxIssueINVData{
		Nonce:       nonce,
	...
	// 调用合约
	_, err = c.CreateAndIssueINV2(transactor, req.TokenId, req.DocumentInfo, req.PONUm, req.DONum, req.INVNUm, req.Farmer, req.Buyer)
	if err != errShouldBe {
		// 这里不可能发生
		err = fmt.Errorf("CreateAndIssueINV2 err=%w", err)
		return
	}
	return data, nil
}
...
func (d *JettradeTxIssueINVData) GetSignBytes() []byte {
	return d.BytesToSign
}

// GetName : impl MessageToSign
func (d *JettradeTxIssueINVData) GetName() string {
	return JettradeTxIssueINVDataName
}

// GetTransportBytes : impl MessageToSign
func (d *JettradeTxIssueINVData) GetTransportBytes() []byte {
...
func (d *JettradeTxIssueINVData) Parse(buf []byte) error {
...
// VerifySignData :
func (d *JettradeTxIssueINVData) VerifySignData(c chain.Chain, account common.Address) (err error) {
	//不做校验,跳过
	return nil
}
